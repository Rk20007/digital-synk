export default function sitemap() {
  const baseUrl = 'https://digitalsynk.com';
  ...
  // Services available on the generic service page (/services/[service])
  ...
  // Services available on the city-specific service page (/services/[service]/[city])
  const cityServices = [
    "seo",
    "local-seo",
    "social-media-marketing",
  ...
  // Generate City Pages (e.g., /delhi)
  const cityRoutes = validCities.map((city) => ({
    url: `${baseUrl}/${city}`,
    lastModified: new Date(),
  ...
  // Generate Generic Service Pages (e.g., /services/seo)
  const serviceRoutes = genericServices.map((service) => ({
  ...
  // Generate Service + City Pages (e.g., /services/seo/delhi)
  const serviceCityRoutes = cityServices.flatMap((service) =>
  ...
  return [...routes, ...cityRoutes, ...serviceRoutes, ...serviceCityRoutes];
}